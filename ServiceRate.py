...
# --- RATE TABLE ---
service_rates = {
    "Service Eng. - Inhouse/Onshore (NT)": 207,
    "Service Eng. - Inhouse/Onshore (OT)": 260,
    "System Eng. - Inhouse/Onshore (NT)": 228,
    "System Eng. - Inhouse/Onshore (OT)": 287,
    "Service Eng. - Accommodated (Day)": 3104,
    "Service Eng. - Accommodated (OT)": 297,
    "System Eng. - Accommodated (Day)": 3428,
    "System Eng. - Accommodated (OT)": 329,
    "Service Eng. - Travel": 164,   # Only one Travel option!
    "System Eng. - Travel": 180,    # Only one Travel option!
    # "Service Eng. - Accommodated (Travel)" 및 "System Eng. - Accommodated (Travel)" 삭제됨
}
...
scope = st.text_area("Write a summary or notes regarding the job scope...", placeholder="e.g., interface IO modification")
...
for i in range(num_lines):
    ...
    with cols[1]:
        ...
        description = st.text_input(
            f"Description {i+1} Label",
            placeholder="e.g., Remote tuning",
            key=f"desc_{i}",
            label_visibility="collapsed"
        )
        st.markdown(
            f"<style>[data-testid='stTextInput'][key='desc_{i}'] input {{max-width:110px !important;}}</style>",
            unsafe_allow_html=True,
        )
    with cols[2]:
